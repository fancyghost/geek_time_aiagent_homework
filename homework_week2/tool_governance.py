...
def _redact(value: Any) -> Any:
    ...
    if isinstance(value, str):
        ...
        # 账号脱敏：保留 ACC-X- 前缀与末 4 位，中间固定 4 个星号（如 ACC-A-****3456）。
        return re.sub(r"(ACC-[A-Z]-)[0-9]{2}([0-9]{4})", r"\g<1>****\g<2>", value)
    return value
...
async def transfer_precheck(raw_arguments: ArgsModel, context: ExecutionContext) -> None:
    ...
    if arguments.amount > 50_000:
        raise PolicyDenied("EXCEED_LIMIT", "单笔转账金额超过限额 50000")
...
async def transfer_handler(
    ...
    # 超时模拟：sleep 位于改余额之前，保证超时时不产生资金副作用。
...
async def run_deepseek_agent(user_input: str) -> None:
    ...
    client = AsyncOpenAI(api_key=api_key, base_url="http://127.0.0.1:8080/v1")
